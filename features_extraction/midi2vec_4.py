'''
# frequency-like features from midi, divided randomly to train set or validation set
# 3 auxiliary features from midi as bellow
# average note num, average intensity, speed
'''
    ...
    while True:
        ...
        if event_type == 255:
            ...
            if 128 <= event_type <= 143:
                ...
                if note_on.get(pitch) is not None:
                    note_list.append(
                        [instrument[event_type % 16], note_on.get(pitch)[0], t, pitch, note_on.get(pitch)[1]])
                    note_on.pop(pitch)
                    # note_event_num += 1
